...
/**
 * Fees smaller than this (in utuc) are considered zero fee (for transaction creation)
 * We are ~100 times smaller then tucoin now (2015-06-23), set minTxFee 10 times higher
 * so it's still 10 times lower comparing to tucoin.
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 TUC.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-tuc-address
                    CScript scriptChange;
