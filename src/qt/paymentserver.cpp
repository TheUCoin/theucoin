...
const int TUCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString TUCOIN_IPC_PREFIX("theucoin:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/theucoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/theucoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/theucoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++) {
        ...
        // If the theucoin: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(TUCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // theucoin: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests) {
        ...
        if (!socket->waitForConnected(TUCOIN_IPC_CONNECT_TIMEOUT)) {
            delete socket;
            socket = nullptr;
            return false;
        }
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(TUCOIN_IPC_CONNECT_TIMEOUT);
                                                                       ...
                                                                       optionsModel(0)
{
    ...
    // on Mac: sent when you click theucoin: links
    ...
    if (startLocalServer) {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start theucoin: click-to-pay handler"));
        } else {
...
//
// OSX-specific way of handling theucoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject* object, QEvent* event)
{
    // clicking on theucoin: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in theucoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(TUCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // theucoin: URI
    {
        ...
        } else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient)) {
                ...
                emit message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid TheUCoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach (const PAIRTYPE(CScript, CAmount) & sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom tuc addresses are not supported
