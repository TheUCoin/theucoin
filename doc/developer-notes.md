...
- [Development guidelines](#development-guidelines)
    - [General TheUCoin Core](#general-theucoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play TUCs (tTUC)" on the test network, if you
...
TheUCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/theucoin-official/theucoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_theucoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_theucoin --log_level=test_suite
$ valgrind -v --leak-check=full src/theucoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_theucoin.coverage/index.html`.
```

**Sanitizers**

TheUCoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/tucoin/tucoin/issues/12691)
...
- BitcoinMiner : Generates tucoins (if wallet is enabled).
...
However, in open source software such as TheUCoin Core, where everyone uses
...
pay attention to for reviewers of TheUCoin Core code.

General TheUCoin Core
--------------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: TheUCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `TUCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef TUCOIN_FOO_BAR_H
#define TUCOIN_FOO_BAR_H
...
#endif // TUCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of TheUCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against TheUCoin Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/tucoin-core/secp256k1/ ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/tucoin-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors it will cause problems with TheUCoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to TheUCoin's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:theucoin/theucoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `theucoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `theucoin-cli` and the GUI debug console use this table to determine how to
