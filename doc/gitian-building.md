...
*Setup instructions for a gitian build of TheUCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TheUCoin
...
- [Building TheUCoin](#building-theucoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-tucoin-host/README.md).
...
Clone the git repositories for theucoin and gitian and then checkout the theucoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the theucoin repository
...
To build TheUCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the theucoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/theucoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/theucoin-official/theucoin
...
For example:
```bash
URL=https://github.com/konez2k/theucoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit theucoin=${COMMIT} --url theucoin=${URL} ../theucoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit theucoin=${COMMIT} --url theucoin=${URL} ../theucoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit theucoin=${COMMIT} --url theucoin=${URL} ../theucoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/theucoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/theucoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/theucoin-build.assert
```
...
[theucoin/gitian.sigs](https://github.com/theucoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
