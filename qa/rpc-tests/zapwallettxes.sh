...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars TUCOIND and TUCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

TUCOIND=${TUCOIND:-${1}/tucoind}
CLI=${TUCOINCLI:-${1}/tucoin-cli}
...
$TUCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$TUCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
