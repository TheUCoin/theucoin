...
# Get BUILDDIR and REAL_TUCOIND
. "${CURDIR}/tests-config.sh"

export TUCOINCLI=${BUILDDIR}/qa/pull-tester/run-tucoin-cli
export TUCOIND=${REAL_TUCOIND}
...
if [ "x${ENABLE_TUCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and tucoind must all be enabled"
fi
