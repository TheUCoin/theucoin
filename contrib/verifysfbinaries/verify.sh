...
###   This script attempts to download the signature file SHA256SUMS.asc from tucoin.org
...
WORKINGDIR="/tmp/tucoin"
TMPFILE="hashes.tmp"

#this URL is used if a version number is not specified as an argument to the script
SIGNATUREFILE="https://tucoin.org/bin/0.9.2.1/SHA256SUMS.asc"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://tucoin.org/bin/"
VERSIONPREFIX="tucoin-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'tucoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[tucoin-]<version>-[rc[0-9]] (example: tucoin-0.9.2-rc1)"
   echo "wget output:"
